...
class ApiService {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    final res = await http
        .post(Uri.parse('$_base/login'), headers: await _headers(auth: false),
  ...
  Future<void> logout() async {
    try {
      await http.post(Uri.parse('$_base/logout'), headers: await _headers()).timeout(_timeout);
    } catch (_) {}
  }
  ...
  Future<List<Questionnaire>> getQuestionnaires({
    ...
    try {
      ...
      if (kodeProvinsi != null)  params['kode_provinsi']  = kodeProvinsi;
      if (kodeKabupaten != null) params['kode_kabupaten'] = kodeKabupaten;
      if (kodeKecamatan != null) params['kode_kecamatan'] = kodeKecamatan;
      if (kodeDesa != null)      params['kode_desa']      = kodeDesa;
      else if (namaDesa != null) params['nama_desa']      = namaDesa;
      if (dusun != null)         params['dusun']          = dusun;
      if (surveyId != null)      params['survey_id']      = surveyId;

      final uri = Uri.parse('$_base/questionnaires').replace(queryParameters: params);
      final res = await http.get(uri, headers: await _headers()).timeout(_timeout);

      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
        final list = body is List ? body : (body['data'] ?? body['items'] ?? []);
  ...
  Future<Questionnaire> createQuestionnaire(Questionnaire q) async {
    final res = await http
        .post(Uri.parse('$_base/questionnaires'), headers: await _headers(), body: jsonEncode(q.toJson()))
        .timeout(_timeout);
  ...
  Future<Questionnaire> updateQuestionnaire(String id, Questionnaire q) async {
    final res = await http
        .put(Uri.parse('$_base/questionnaires/$id'), headers: await _headers(), body: jsonEncode(q.toJson()))
        .timeout(_timeout);
    if (res.statusCode == 200) return Questionnaire.fromJson(jsonDecode(res.body));
    _throw(res, 'Gagal memperbarui kuesioner');
  }

  Future<void> deleteQuestionnaire(String id) async {
    final res = await http
        .delete(Uri.parse('$_base/questionnaires/$id'), headers: await _headers())
        .timeout(_timeout);
  ...
  Future<SyncResult> syncPendingData() async {
    if (!await hasConnection()) return SyncResult(success: false, message: 'Tidak ada koneksi internet');
    final pending = await StorageService.instance.getPendingQuestionnaires();
    int synced = 0;
    final errors = <String>[];
    for (final row in pending) {
      try {
        final q = Questionnaire.fromJson(jsonDecode(row['data'] as String));
        final saved = await createQuestionnaire(q);
        await StorageService.instance.markSynced(row['id'] as int, saved.id ?? '0');
        synced++;
      } catch (e) {
        errors.add('$e');
    ...
    return SyncResult(success: errors.isEmpty, message: 'Sinkronisasi $synced dari ${pending.length} data',
        synced: synced, total: pending.length, errors: errors);
  }
}
...
class SyncResult {
  ...
  SyncResult({required this.success, required this.message, this.synced = 0, this.total = 0, this.errors = const []});
}